...
local function EnsureResultsFrame(self)
    ...
    f:SetSize(720, 640) -- bigger window
    f:SetPoint("CENTER")
    f:Hide()
...
function UPA.ui:RenderStepsPage(f, page)
    ...
    -- Steps rendering (paged)
    local c = f.content
    ClearChildren(c)
    local y = -4
    local lineH = 24
    local function addStep(txt, font, r,g,b)
        local fs = c:CreateFontString(nil, "OVERLAY", font or "GameFontNormalLarge")
...
function UPA.ui:RenderPath(f, skill, target)
    ...
    if not data or not data.steps or #data.steps == 0 then
        ClearChildren(f.content); f.pageLabel:SetText("")
        f.totals:SetText("Totals: —")
        f.missing:SetText("")
        ClearChildren(f.shopContent)
        return
    end
    ...
    -- Shopping list rendering (not paged, still scrollable)
    local sc = f.shopContent
    ClearChildren(sc)
    local sy = -4
    local lineH2 = 24
    local function addShop(txt, font, r,g,b)
        local fs = sc:CreateFontString(nil, "OVERLAY", font or "GameFontNormalLarge")
